...
def plot_velocity(ax, model, source=None, receiver=None, colorbar=True, depth_scale=1.):
    """
    ...
    domain_size = 1.e-3 * np.array(model.domain_size)
    extent = [model.origin[0], model.origin[0] + domain_size[0],
              model.origin[1] + depth_scale*domain_size[1], model.origin[1]]

    plot = ax.imshow(np.transpose(model.vp), animated=True, cmap=cm.jet,
    ...
    if receiver is not None:
        plt.scatter(1e-3*receiver[:, 0], depth_scale*1e-3*receiver[:, 1],
                    s=25, c='green', marker='D')

    # Plot receiver points, if provided
    if source is not None:
        plt.scatter(1e-3*source[:, 0], depth_scale*1e-3*source[:, 1],
                    s=25, c='red', marker='o')

    # Ensure axis limits
    ax.set_xlim(model.origin[0], model.origin[0] + domain_size[0])
    ax.set_ylim(model.origin[1] + depth_scale*domain_size[1], model.origin[1])
...
def plot_shotrecord(ax, rec, model, t0, tn, colorbar=True, time_scale=1., scale=None, scale_x=1.):
    """
    ...
    extent = [model.origin[0], model.origin[0] + scale_x*1e-3*model.domain_size[0],
              time_scale*1e-3*tn, t0]

    plot = ax.imshow(rec[::-1], vmin=-scale, vmax=scale, cmap=cm.gray, extent=extent)

    ax.set_xlim(model.origin[0], model.origin[0] + scale_x*1e-3*model.domain_size[0])
    ax.set_ylim(t0, time_scale*1e-3*tn)
